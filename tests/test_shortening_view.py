"""Тесты главной страницы и перенаправления по коротким ссылкам."""
...
def test_index_form_get(client):
    """Проверить отображение формы на главной странице."""
...
def test_index_form_post(client):
    """Проверить создание короткой ссылки через форму."""
...
def test_duplicated_url_in_form(client, duplicated_custom_id_msg,
                                short_python_url):
    """Проверить сообщение при повторном идентификаторе."""
    response = client.post(INDEX_URL, data={
        'original_link': PY_URL,
        'custom_id': CUSTOM_ID,
...
def test_files_as_a_short_link(client, duplicated_custom_id_msg):
    """Проверить запрет служебного идентификатора files."""
    response = client.post(INDEX_URL, data={
        'original_link': PY_URL,
        'custom_id': 'files',
    }, follow_redirects=True)
...
def test_get_unique_short_id(client):
    """Проверить создание идентификатора для пустого поля формы."""
    asser_msg_pattern = (
...
def test_redirect_url(client, short_python_url):
    """Проверить перенаправление по короткой ссылке."""
...
def test_len_short_id_form(client):
    """Проверить ограничение длины идентификатора в форме."""
    assert_msg_pattern = (
...
def test_len_short_id_autogenerated_view(client):
    """Проверить длину созданного сервисом идентификатора."""
...
def test_invalid_short_url(data, client):
    """Проверить отклонение идентификатора с запрещёнными символами."""
