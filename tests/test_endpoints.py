"""Тесты эндпоинтов API для работы с короткими ссылками."""
...
def test_create_id(client):
    """Проверить создание ссылки с пользовательским идентификатором."""
...
def test_create_empty_body(client):
    """Проверить обработку запроса без JSON-тела."""
...
def test_invalid_short_url(json_data, client):
    """Проверить отклонение недопустимого короткого идентификатора."""
...
def test_no_required_field(client):
    """Проверить ошибку при отсутствии обязательного поля url."""
    try:
        response = client.post(CREATE_SHORT_LINK_URL, json={
            'short_link': 'python',
        })
...
def test_url_already_exists(client, short_python_url,
    ...
    """Проверить ошибку при повторном коротком идентификаторе."""
    try:
        response = client.post(CREATE_SHORT_LINK_URL, json={
...
def test_generated_unique_short_id(json_data, client):
    """Проверить автоматическое создание уникального идентификатора."""
    assert_msg_pattern = (
...
def test_get_url_endpoint(client, short_python_url):
    """Проверить получение исходного URL по идентификатору."""
    response = client.get(
...
def test_get_url_not_found(client):
    """Проверить ответ для отсутствующего короткого идентификатора."""
    assert_msg_pattern = (
...
def test_len_short_id_api(client):
    """Проверить ограничение длины пользовательского идентификатора."""
    assert_msg_pattern = (
...
def test_len_short_id_autogenerated_api(client):
    """Проверить длину автоматически созданного идентификатора."""
